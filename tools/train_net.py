...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, index, time,
        ...
        # len(inputs) = num_clips, inputs[0][0].shape = [4, 3, 8, 224, 224]
        if cfg.NUM_GPUS:
            # inputs: n_clips * [[bs, 3, 8, 224, 224]]
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            if cfg.TRAIN.VIS_INPUT:
                ...
                for clip_idx in range(len(inputs)):
                    # [bs, 3, 8, 224, 224]
                    batch_frames = inputs[clip_idx][0].cpu()
                    for video_id in range(batch_frames.shape[0]):
                        imgs = batch_frames[video_id]
                        imgs = [
                            ToPILImage()(imgs[:, i])
                            for i in range(imgs.shape[1])
                        ]
                        video_path = f"{cfg.OUTPUT_DIR}/vis/train_iter{cur_iter}_vid{video_id}_clip{clip_idx}/"
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                for key in partial_other_loss:
                    partial_other_loss[key] = partial_other_loss[key].item()
        ...
        train_meter.log_iter_stats(cur_epoch, cur_iter, partial_other_loss)
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.TASK == "ssl" and cfg.MODEL.MODEL_NAME == "ContrastiveModel":
                ...
                yd, yi, q_knn = model(inputs, index, time)
                K = yi.shape[1]
...
def calculate_and_update_precise_bn(loader,
                                    model,
                                    num_iters=200,
                                    use_gpu=True):
    """
...
def train(cfg):
    ...
    if (start_epoch == cfg.SOLVER.MAX_EPOCH
        ...
        eval_epoch(val_loader, model, val_meter, start_epoch, cfg,
                   train_loader, writer)
    if writer is not None:
        writer.close()
    result_string = (
        ...
        "".format(
            ...
            60.0 if len(epoch_timer.epoch_times) else 0.0,
            misc.gpu_mem_usage(),
