...
class ContrastiveModel(nn.Module):
    """
    Contrastive Model with a fine-grained key-value memory-enhanced predictor.
    The implementation if modified from https://github.com/facebookresearch/SlowFast/blob/main/slowfast/models/contrastive.py.

    Args:
        cfg (CfgNode): Configuration settings for the model.

    Attributes:
        backbone (nn.Module): The backbone architecture of the model.
        type (str): The type of contrastive model (e.g., "mem", "moco", "swav"， "byol").
        T (int): Temperature parameter for contrastive loss.
        dim (int): Dimensionality of the contrastive feature embeddings.
        length (int): Length of memory queue for contrastive loss.
        k (int): Length of the memory queue for the "moco" type.
        mmt (float): Momentum for updating memory bank.
        momentum_annealing (bool): Whether to anneal momentum during training.
        duration (int): Duration of video clips.
        cfg (CfgNode): Model configuration settings.
        num_gpus (int): Number of GPUs.
        l2_norm (Normalize): L2 normalization layer.
        knn_num_imgs (int): Number of images used for k-NN updates.
        knn_on (bool): Whether k-NN updates are enabled.
        train_labels (numpy.ndarray): Training labels.
        num_pos (int): Number of positive samples for contrastive loss.
        num_crops (int): Number of spatial-temporal crops.
        nce_loss_fun (nn.Module): Contrastive loss function.
        softmax (nn.Module): Softmax function for contrastive loss.
        predictor_type (str): Type of the predictor used.
        mem_type (str): Type of memory used for contrastive learning ("1d" or "2d").
        memory (Memory1D or Memory): Memory bank for contrastive learning.
        examplar_type (str): Type of exemplar used ("video" or other).
        interp (bool): Whether to interpolate memory bank embeddings.
    """
    ...
    def __init__(self, cfg):
        ...
        if self.type == "mem":
            ...
            self._batch_shuffle_on = (False if
                                      ...
                                      or self.type in ["byol"] else True)
        elif self.type == "swav":
    ...
    def init_knn_labels(self, train_loader):
        ...
        if self.length != self.num_imgs:
            logger.error(
                "Kinetics dataloader size: {} differs from memorybank length {}"
                .format(self.num_imgs, self.length))
    ...
    def eval_knn(self, q_knn, knn_k=200):
        with torch.no_grad():
            if (q_knn.view(q_knn.size(0), -1).shape[-1]
                    != self.knn_mem.memory.view(self.knn_mem.memory.size(0),
                                                -1).shape[-1]):
                dist = torch.zeros(q_knn.size(0),
                                   self.knn_mem.memory.size(0)).type_as(q_knn)
            else:
                dist = torch.einsum(
                    "nc,mc->nm",
    ...
    def sim_loss(self, q, k):
        # Modification by Xiaojie Li: Normalize input tensors
        similarity = torch.einsum(
            "nc,nc->n", [F.normalize(q, dim=-1),
                         F.normalize(k, dim=-1)])  # N-dim
    ...
    def momentum_anneal_cosine(self, epoch_exact):
        self.mmt = (
            1 - (1 - self.cfg.CONTRASTIVE.MOMENTUM) *
    ...
    def _dequeue_and_enqueue(self, keys, extra_keys=None):
        ...
        if (not self.cfg.CONTRASTIVE.MOCO_MULTI_VIEW_QUEUE
            ):  # TODO: add multiview negatives
            keys_queue_update = [keys[0]]
        else:
            assert (
                len(keys)
                > 0), "need to have multiple views for adding them to queue"
            keys_queue_update = []
        ...
        for i, clip in enumerate(clips):
            for j, view in enumerate(clip):
                if i == 0:
                    ...
                    clips_batched[j] = torch.cat([clips_batched[j], view],
                                                 dim=0)
                del view
    ...
    def compute_key_feat(
        ...
        """
           Compute key features from video clips.

           Args:
               clips_k (List[Tensor]): List of video clips.
               compute_predictor_keys (bool): Whether to compute predictor keys.
               batched_inference (bool): Whether to use batched inference.
               norm_feat (bool): Whether to normalize the features.

           Returns:
               keys (List[Tensor]): List of computed keys.
               pred_keys (List[Tensor] or None): List of computed predictor keys, if enabled.
               pred_maps (List[Tensor] or None): List of computed feature maps, if enabled.
           """
        ...
        if batched_inference and all([
                ...
                for i in range(len(clips_k)) for j in range(len(clips_k[i]))
        ]):
        ...
        for k in range(0, len(clips_k)):
            ...
            with torch.no_grad():
                if self.cfg.CONTRASTIVE.PREDICTOR_TYPE in ["MemoryRecon"]:
                    hist_feat, maps = self.backbone_hist(clip_k)
                    pred_maps.append(maps)
                else:
                ...
                if norm_feat:
                    ...
                    x_hist = hist_feat
                if self._batch_shuffle_on:
                    x_hist = self._batch_unshuffle(x_hist,
                                                   idx_restore).detach()
            keys.append(x_hist)
        ...
        if compute_predictor_keys and self.cfg.CONTRASTIVE.PREDICTOR_TYPE in [
                "MemoryRecon"
        ]:
            return keys, pred_keys, pred_maps
        elif compute_predictor_keys:
            return keys, pred_keys
        elif self.cfg.CONTRASTIVE.PREDICTOR_TYPE in ["MemoryRecon"]:
            return keys, pred_maps
        else:
            return keys
        # XiaoJieLi Modified: Added 'MemoryRecon' related content.
        # XiaoJieLi Modified: Added 'pred_maps' to return if 'MemoryRecon' is enabled.
        # XiaoJieLi Modified: Added 'norm_feat' parameter for feature normalization.
    ...
    def forward(
        ...
        # Check if the model type is 'mem'
        if self.type == "mem":
            # Rest of the code for 'mem' type remains unchanged
            ...
            if self.mem_type == "2d":
                if self.interp:
                    time_ind = (torch.empty(batch_size, self.k + 1).uniform_(
                        0, self.duration - 1).cuda())
                else:
                ...
                time_ind = torch.zeros(size=(batch_size, self.k + 1),
                                       dtype=int).cuda()
        ...
        # Check if the model type is 'moco'
        elif self.type == "moco":
            # Rest of the code for 'moco' type remains unchanged
            if isinstance(clips[0], list):
                n_clips = len(clips)
                ind_clips = np.arange(
                    n_clips)  # clips come ordered temporally from decoder

                clip_q = clips[ind_clips[0]]
                clips_k = [clips[i] for i in ind_clips[1:]]
                # rearange time
                time_q = time[:, ind_clips[0], :]
                time_k = (time[:, ind_clips[1:], :]
                          if keys is None else time[:, ind_clips[0] + 1:, :])
            else:
                clip_q = clips
            ...
            if keys is None:
                keys = self.compute_key_feat(clips_k,
                                             compute_predictor_keys=False)
            ...
            # score computation
            queue_neg = torch.einsum("nc,kc->nk",
                                     [q, self.queue_x.clone().detach()])
        ...
        # Check if the model type is 'byol' (with BYOL-specific modifications)
        elif self.type in ["byol"]:
            # Rest of the code for 'byol' type remains unchanged
            # time.shape = [batch_size, n_clips, 3]
            # clips: n_clips(n_time * n_spatial) * 1 * [[bs, 3, 8, 224, 224]]
            ...
            if isinstance(clips[0], list):
                ...
                clip_q = clips
            if self.cfg.CONTRASTIVE.PREDICTOR_TYPE in ["MemoryRecon"]:
                feat_q, maps = self.backbone(clip_q,
                                             infos=[keys, time, pred_maps])
            else:
                feat_q = self.backbone(clip_q, infos=[keys, time])
            # len(feat_q) = 0, feat[0] = projections, feat[1:] = predictors
            predictors = []
            loss_memory = {}
            vis_feature = {}
            if isinstance(feat_q, list):
                if isinstance(feat_q[1], dict):
                    for i in range(len(feat_q[1:])):
                        predictors.append(feat_q[i + 1]["output_predict"])
                        for key in feat_q[i + 1]:
                            if key.find("loss") >= 0:
                                loss_memory[key] = feat_q[i + 1][key]
                            if (self.cfg.TEST.VIS_MIDDLE
                                    and key.find("vis") >= 0):
                                vis_feature[key] = feat_q[i + 1][key]
                else:
                    predictors = feat_q[1:]
                feat_q = feat_q[0]
            ...
            if not self.training:
                if self.cfg.CONTRASTIVE.EXTRACT_TYPE == "maps":
                    return self.eval_knn(feat_q)
                elif self.cfg.TEST.VIS_MIDDLE:
                    return vis_feature
                else:
                    return self.eval_knn(q_knn)
            ...
            if keys is None:
                keys = self.compute_key_feat(
                    ...
                    compute_predictor_keys=False,
                    norm_feat=True
                    if self.cfg.CONTRASTIVE.NORM_KEYS else False,
                )
                if self.cfg.CONTRASTIVE.PREDICTOR_TYPE in ["MemoryRecon"]:
                    keys, pred_maps = keys
                auto_enqueue_keys = True
            ...
            # calculate loss
            loss_vis = {}
            if not self.cfg.CONTRASTIVE.NORM_KEYS:
                seperate_loss = torch.abs(
                    torch.eye(keys[0].shape[0]).cuda() - torch.matmul(
                        F.normalize(keys[0], dim=-1),
                        F.normalize(keys[0], dim=-1).transpose(0, 1),
                    )).mean()
            else:
                seperate_loss = torch.abs(
                    torch.eye(keys[0].shape[0]).cuda() -
                    torch.matmul(keys[0], keys[0].transpose(0, 1))).mean()
            loss_vis["seperate_loss"] = seperate_loss.detach().clone()
            ...
            if self.cfg.CONTRASTIVE.SEQUENTIAL:
                ...
                if batch_clips:
                    ...
                    loss_reg = self.sim_loss(predictors[0][:bs, :],
                                             keys[0][bs:, :]) + self.sim_loss(
                                                 predictors[0][bs:, :],
                                                 keys[0][:bs, :])
                    q_knn = q_knn[:bs, :]
                    del clips_batched[0]
            ...
            loss_vis["loss_reg"] = loss_reg.detach().clone()
            # loss_pos = self.sim_loss(q1_proj, q2_proj)
            dummy_logits = torch.cat(
                (
                    9999.0 * torch.ones(
                        (len(index), 1), dtype=torch.float).cuda(),
                    torch.zeros(
                        (len(index), self.k), dtype=torch.float).cuda(),
                ),
                dim=1,
            )
            ...
            return dummy_logits, loss_reg, loss_memory, loss_vis
            ...
            if self.swav_use_public_code:
                ...
                self.swav_crops_for_assign = np.arange(n_clips -
                                                       swav_extra_crops)
                    ...
                    with torch.no_grad():
                        ...
                        if (self.cfg.CONTRASTIVE.SWAV_QEUE_LEN > 0
                            ...
                            self.queue_swav[i, bs:] = self.queue_swav[
                                i, :-bs].clone()
                            self.queue_swav[i, :bs] = embedding[crop_id *
                                                                bs:(crop_id +
                                                                    1) * bs]
                        q = out / self.swav_eps_sinkhorn
                        q = torch.exp(q).t()
                        q = (self.distributed_sinkhorn(q, 3)[-bs:]
                             if self.cfg.NUM_SHARDS > 1 else self.sinkhorn(
                                 q.t(), 3)[-bs:])
                    subloss = 0
                ...
                if self.cfg.CONTRASTIVE.SWAV_QEUE_LEN > 0:
                    if epoch_exact >= 15.0 and not torch.all(
                        ...
                        swav_prototypes = F.normalize(self.swav_prototypes,
                                                      dim=0,
                                                      p=2).detach()
                        out_1 = torch.cat((
                            torch.mm(self.queue_swav[0].detach(),
                                     swav_prototypes),
                            out_1,
                        ))
                        out_2 = torch.cat((
                            torch.mm(self.queue_swav[1].detach(),
                                     swav_prototypes),
                            out_2,
                        ))
                    # fill the queue
            ...
            if distributed_loss and self.cfg.NUM_GPUS > 1:
                ...
                for loss_id in range(len(self.pos_mask)):
                    ...
                    idx = (1 - torch.sum(self.pos_mask[loss_id], 1)
                           > 0).detach()
...
class Normalize(nn.Module):
    ...
    def forward(self, x):
        norm = (x.pow(self.power).sum(self.dim,
                                      keepdim=True).pow(1.0 / self.power))
        out = x.div(norm)
        return out
...
class Memory(nn.Module):
    ...
    def resize(self, length, duration, dim):
        ...
        stdv = 1.0 / math.sqrt(dim / 3)
        del self.memory
        self.memory = (torch.rand(length, duration, dim,
                                  device=self.device).mul_(
                                      2 * stdv).add_(-stdv).cuda())
    ...
    def update(self, mem, momentum, ind, time, interp=False):
        ...
        with torch.no_grad():
            if interp:
                ...
                duo_update = False
                if duo_update:
                    update_t0 = (mem * w_t0 + mem_t0 *
                                 w_t1) * momentum + mem_t0 * (1 - momentum)
                    update_t1 = (mem * w_t1 + mem_t1 *
                                 w_t0) * momentum + mem_t1 * (1 - momentum)
                else:
                ...
                # my version
                self.memory[ind.view(-1),
                            time.long().view(-1), :] = mem_update.squeeze()
                return
...
class Memory1D(nn.Module):
    ...
    def __init__(self, length, duration, dim, cfg):
        ...
        self.register_buffer(
            "memory",
            torch.rand(length, dim).mul_(2 * stdv).add_(-stdv))
        self.l2_norm = Normalize(dim=1)
        self.num_gpus = cfg.NUM_GPUS
    ...
    def get(self, ind, time, interp=False):
        batch_size = ind.size(0)
        if len(ind.shape) == 1:
            return torch.index_select(self.memory, 0,
                                      ind.view(-1)).view(batch_size, self.dim)
        else:
...
def contrastive_parameter_surgery(model, cfg, epoch_exact, cur_iter):
    ...
    if (cfg.MODEL.MODEL_NAME == "ContrastiveModel"
            and cfg.CONTRASTIVE.TYPE == "swav" and epoch_exact <= 1.0):
    ...
    if (cfg.MODEL.MODEL_NAME == "ContrastiveModel"
        ...
        assert (cfg.CONTRASTIVE.QUEUE_LEN %
                (cfg.TRAIN.BATCH_SIZE * cfg.NUM_SHARDS) == 0)
...
def contrastive_forward(model, cfg, inputs, index, time, epoch_exact, scaler):
    # inputs: n_clips * [batch_size, 3, 8, 224, 224]
    if cfg.CONTRASTIVE.SEQUENTIAL:
        perform_backward = False
        mdl = model.module if hasattr(model, "module") else model
        keys = (mdl.compute_key_feat(
            inputs,
            compute_predictor_keys=False,
            batched_inference=True if len(inputs) < 2 else False,
            norm_feat=True if cfg.CONTRASTIVE.NORM_KEYS else False,
        ) if cfg.CONTRASTIVE.TYPE in ["moco", "byol"] else [None] *
                len(inputs))
        ...
        # keys: n_clips * [4, 256]
        # time: [batchsize, n_clips, ?] e.g. [4, 2, 3]
        partial_other_loss = {}
        partial_loss = torch.zeros(1).float().cuda()
        for k, vid in enumerate(inputs):
            other_keys = keys[:k] + keys[k + 1:]
            if pred_maps is not None:
                other_pred_maps = pred_maps[:k] + pred_maps[k + 1:]
            else:
                other_pred_maps = None
            time_cur = torch.cat(
                [
                    time[:, k:k + 1, :],
                    time[:, :k, :],
            ...
            vids = [vid]
            if (cfg.CONTRASTIVE.TYPE == "swav"
                    or cfg.CONTRASTIVE.TYPE == "simclr"):
                if k < len(inputs) - 1:
                    vids = inputs[k:k + 2]
                else:
                    break
            lgt_k, loss_k, loss_memory_k, loss_vis = model(
                vids,
                index,
                time_cur,
                epoch_exact,
                keys=other_keys,
                pred_maps=other_pred_maps,
            )
            loss_backward = torch.zeros(1).float().cuda()
            if cfg.CONTRASTIVE.LOSS_SIM_WEIGHT != 0:
                loss_backward += loss_k
            for key in loss_memory_k:
                if cfg.MEMORY[key.upper()] != 0:
                    loss_backward += (loss_memory_k[key] *
                                      cfg.MEMORY[key.upper()])
            is_nan = misc.check_nan_losses(loss_backward)
            if not is_nan:
                scaler.scale(loss_backward).backward()
            if k == 0:
                preds, partial_loss = lgt_k, loss_k.detach().clone()
                for key in loss_memory_k:
                    partial_other_loss[key] = (
                        loss_memory_k[key].detach().clone())
                for key in loss_vis:
                    partial_other_loss[key] = loss_vis[key].detach().clone()
            else:
                preds = torch.cat([preds, lgt_k], dim=0)
                partial_loss += loss_k.detach().clone()
                for key in loss_memory_k:
                    if key in partial_other_loss:
                        partial_other_loss[key + f"_{k}"] = (
                            loss_memory_k[key].detach().clone())
                    else:
                        partial_other_loss[key] = loss_memory_k[key].detach()
                for key in loss_vis:
                    if key in partial_other_loss:
                        partial_other_loss[key + f"_{k}"] = (
                            loss_vis[key].detach().clone())
                    else:
                        partial_other_loss[key] = loss_vis[key].detach()
        partial_loss /= len(inputs) * 2.0  # to have same loss as symm model
        # for key in partial_other_loss:
        #     partial_other_loss[key] /= len(inputs) * 2.0
        if cfg.CONTRASTIVE.TYPE == "moco":
            mdl._dequeue_and_enqueue(keys)
    else:
        perform_backward = True
        partial_other_loss = {}
        preds, partial_loss, loss_memory_k, loss_vis = model(inputs,
                                                             index,
                                                             time,
                                                             epoch_exact,
                                                             keys=None)
        ...
        for key in loss_vis:
            partial_other_loss[key] = loss_vis[key].detach()
    return model, preds, partial_loss, perform_backward, partial_other_loss
