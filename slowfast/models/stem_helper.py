...
class R2Plus1dStem(nn.Module):
    """
    R(2+1)D stem is different than the default one as it uses separated
    3D convolution
    """
    ...
    def _construct_stem(self, dim_in, dim_out, norm_module):
        dim_mid = 83  #45 # hard code by xiaojie
        ...
        self.bn1 = norm_module(num_features=dim_mid,
                               eps=self.eps,
                               momentum=self.bn_mmt)
        ...
        self.bn2 = norm_module(num_features=dim_out,
                               eps=self.eps,
                               momentum=self.bn_mmt)
        self.relu = nn.ReLU(self.inplace_relu)
