    ...
    if cfg.SOLVER.LAYER_DECAY > 0.0 and cfg.SOLVER.LAYER_DECAY < 1.0:
        ...
        for name_m, m in model.named_modules():
            ...
            for name_p, p in m.named_parameters(recurse=False):
                ...
                elif cfg.SOLVER.ZERO_WD_1D_PARAM and (len(p.shape) == 1 or
                                                      name.endswith(".bias")):
                    zero_parameters.append(p)
                else:
        ...
        raise ValueError("Layer decay should be in (0, 1], but is {}".format(
...
def get_param_groups(model, cfg):
    ...
    for name, p in model.named_parameters():
        ...
        if name in skip or ((len(p.shape) == 1 or name.endswith(".bias"))
    ...
    # Check all parameters will be passed into optimizer.
    assert (len(list(model.parameters())) == non_bn_parameters_count +
            zero_parameters_count + no_grad_parameters_count
            ), "parameter size does not match: {} + {} + {} != {}".format(
    ...
    print("non bn {}, zero {}, no grad {}".format(
        non_bn_parameters_count,
        zero_parameters_count,
        no_grad_parameters_count,
    ))
