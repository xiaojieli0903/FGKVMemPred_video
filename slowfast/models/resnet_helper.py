...
def get_trans_func(name):
    ...
    trans_funcs = {
        ...
        "basic_transform_r3d": BasicTransformR3D,
        "basic_transform_r2plus1d": BasicTransformR2Plus1d,
    }
...
class BasicTransformR3D(nn.Module):
    """
    Basic transformation: Tx3x3, 1x3x3, where T is the size of temporal kernel.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            norm_module (nn.Module): nn.Module for the normalization layer. The
                default is nn.BatchNorm3d.
        """
        super(BasicTransformR3D, self).__init__()
        self.temp_kernel_size = temp_kernel_size
        self._inplace_relu = inplace_relu
        self._eps = eps
        self._bn_mmt = bn_mmt
...
class BasicTransformR2Plus1d(nn.Module):
    """
    Basic transformation: Tx3x3, 1x3x3, where T is the size of temporal kernel.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            norm_module (nn.Module): nn.Module for the normalization layer. The
                default is nn.BatchNorm3d.
        """
        super(BasicTransformR2Plus1d, self).__init__()
        self._inplace_relu = inplace_relu
        self._eps = eps
        self._bn_mmt = bn_mmt
        self._construct(dim_in, dim_out, stride, dilation, norm_module)
    ...
    def _construct(self, dim_in, dim_out, stride, dilation, norm_module):
        dim_mid = (dim_in * dim_out * 3 * 3 * 3) // (dim_in * 3 * 3 +
                                                     3 * dim_out)
        # Tx3x3, BN, ReLU.
        self.a = nn.Conv3d(
            dim_in,
        ...
        self.a_bn = norm_module(num_features=dim_mid,
                                eps=self._eps,
                                momentum=self._bn_mmt)
        self.a_relu = nn.ReLU(inplace=self._inplace_relu)
        self.a_conv2 = nn.Conv3d(
            dim_mid,
            dim_out,
            kernel_size=[3, 1, 1],
            stride=[stride, 1, 1],
        ...
        self.a_bn1 = norm_module(num_features=dim_out,
                                 eps=self._eps,
                                 momentum=self._bn_mmt)
        dim_mid = (dim_out * dim_out * 3 * 3 * 3) // (dim_out * 3 * 3 +
                                                      3 * dim_out)
        # 1x3x3, BN.
        self.b = nn.Conv3d(
            dim_out,
        ...
        self.b_bn = norm_module(num_features=dim_mid,
                                eps=self._eps,
                                momentum=self._bn_mmt)
        self.b_relu = nn.ReLU(inplace=self._inplace_relu)
        self.b_conv2 = nn.Conv3d(
            dim_mid,
            dim_out,
            kernel_size=[3, 1, 1],
            stride=[1, 1, 1],
        ...
        self.b_bn1 = norm_module(num_features=dim_out,
                                 eps=self._eps,
                                 momentum=self._bn_mmt)
...
class ResBlock(nn.Module):
    ...
    def _construct(
        ...
        if (dim_in != dim_out) or (stride != 1):
            if trans_func in [BasicTransformR3D]:
            ...
            elif trans_func in [BasicTransformR2Plus1d]:
                dim_down_mid = (dim_in * dim_out) // (dim_in + dim_out)
                self.branch1 = nn.Sequential(
                    ...
                    norm_module(
                        num_features=dim_down_mid,
                        eps=self._eps,
                        momentum=self._bn_mmt,
                    ),
                    nn.ReLU(inplace=self._inplace_relu),
                    nn.Conv3d(
                        dim_down_mid,
                        dim_out,
                        kernel_size=1,
...
class ResStage(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            trans_func_name (string): name of the transformation function apply
                on the network.
            norm_module (nn.Module): nn.Module for the normalization layer. The
                default is nn.BatchNorm3d.
            drop_connect_rate (float): basic rate at which blocks are dropped,
    ...
    def forward(self, inputs, extract=False):
        output = []
        for pathway in range(self.num_pathways):
            x = inputs[pathway]
            for i in range(self.num_blocks[pathway]):
                m = getattr(self, "pathway{}_res{}".format(pathway, i))
                x = m(x, extract=extract)
