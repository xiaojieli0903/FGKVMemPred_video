...
def construct_loader(cfg, split, is_precise_bn=False):
    ...
    """
    assert split in [
        "train", "val", "test", "retrieval_train", "retrieval_val"
    ]
    if split in ["train"]:
        ...
        shuffle = True
        drop_last = True
    elif split in ["val", "retrieval_train", "retrieval_val"]:
        dataset_name = cfg.TRAIN.DATASET
        batch_size = int(cfg.TRAIN.BATCH_SIZE / max(1, cfg.NUM_GPUS))
        shuffle = False
        drop_last = False
    ...
    if isinstance(dataset, torch.utils.data.IterableDataset):
        ...
        if (cfg.MULTIGRID.SHORT_CYCLE and split in ["train"]
            ...
            if cfg.DETECTION.ENABLE:
                ...
                (cfg.AUG.NUM_SAMPLE > 1 or cfg.DATA.TRAIN_CROP_NUM_TEMPORAL > 1
                 or cfg.DATA.TRAIN_CROP_NUM_SPATIAL > 1)
                    and split in ["train"]
                    and not cfg.MODEL.MODEL_NAME == "ContrastiveModel"):
                collate_func = partial(multiple_samples_collate,
                                       fold="imagenet" in dataset_name)
            else:
                collate_func = None
            loader = torch.utils.data.DataLoader(
                dataset,
