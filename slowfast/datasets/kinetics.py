...
from slowfast.datasets.transform import MaskingGenerator, MaskingGenerator3D, create_random_augment
...
class Kinetics(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=100):
        ...
        self.use_chunk_loading = (True if self.mode in ["train"]
                                  and self.cfg.DATA.LOADER_CHUNK_SIZE > 0 else
                                  False)
        ...
        # the frames.
        if self.mode in ["train", "val", "retrieval_train", "retrieval_val"]:
            self._num_clips = 1
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ["train", "val", "retrieval_train", "retrieval_val"]:
            ...
            if self.mode.find("retrieval") >= 0:
                # center crop from a bigger size without a random rescale
                spatial_sample_index = -1
                min_scale = self.cfg.DATA.TRAIN_JITTER_SCALES[0]  # 128
                max_scale = self.cfg.DATA.TRAIN_JITTER_SCALES[1]  # 160
                crop_size = self.cfg.DATA.TRAIN_CROP_SIZE  # 128
            else:
                spatial_sample_index = -1
            ...
            #)
            spatial_sample_index = (-1)
            # for retrieval: resize 128, crop 112
            ...
            raise NotImplementedError("Does not support {} mode".format(
                self.mode))
        num_decode = (self.cfg.DATA.TRAIN_CROP_NUM_TEMPORAL
        ...
        if len(min_scale) < num_decode:
            ...
            crop_size += ([self.cfg.MULTIGRID.DEFAULT_S] *
                          (num_decode - len(crop_size))
                          if self.cfg.MULTIGRID.LONG_CYCLE
                          or self.cfg.MULTIGRID.SHORT_CYCLE else
        ...
        for i_try in range(self._num_retries):
            ...
            if self.mode in ["train"]:
                assert (len(min_scale) == len(max_scale) == len(crop_size) ==
                        num_decode)

            target_fps = self.cfg.DATA.TARGET_FPS
            if self.cfg.DATA.TRAIN_JITTER_FPS > 0.0 and self.mode in ["train"]:
                target_fps += random.uniform(0.0,
                                             self.cfg.DATA.TRAIN_JITTER_FPS)
            ...
            frames, time_idx, tdiff = decoder.decode(
                ...
                video_meta=self._video_meta[index] if len(self._video_meta)
                < 5e6 else {},  # do not cache on huge datasets
                target_fps=target_fps,
                backend=self.cfg.DATA.DECODING_BACKEND,
                use_offset=self.cfg.DATA.USE_OFFSET_SAMPLING
                if self.mode.find("retrieval") < 0 else True,
                max_spatial_scale=min_scale[0] if all(x == min_scale[0]
                                                      for x in min_scale) else
                0,  # if self.mode in ["test"] else 0,
                time_diff_prob=self.p_convert_dt
                if self.mode in ["train"] else 0.0,
            ...
            if (self.cfg.DATA.CHOOSE_ONE_CLIP > 0
                    and random.random() < self.cfg.DATA.CHOOSE_ONE_CLIP):
                chosen_idx = random.randint(0, num_decode - 1)
                print(f"choose clip {chosen_idx}")
                frames = [frames[chosen_idx] for _ in range(num_decode)]
            ...
            num_aug = (self.cfg.DATA.TRAIN_CROP_NUM_SPATIAL *
            ...
            for i in range(num_decode):
                for _ in range(num_aug):
                    ...
                    relative_scales = (None if (self.mode not in ["train"]
                                                or len(scl) == 0) else scl)
                    relative_aspect = (None if (self.mode not in ["train"]
                                                or len(asp) == 0) else asp)
                    f_out[idx] = utils.spatial_sampling(
                        ...
                        inverse_uniform_sampling=self.cfg.DATA.
                        INV_UNIFORM_SAMPLE,
                        aspect_ratio=relative_aspect,
                        scale=relative_scales,
                        motion_shift=self.cfg.DATA.TRAIN_JITTER_MOTION_SHIFT
                        if self.mode in ["train"] else False,
                    )
                    ...
                    if self.rand_erase:
                        ...
                        f_out[idx] = erase_transform(f_out[idx].permute(
            ...
            logger.warning("Failed to fetch video after {} retries.".format(
                self._num_retries))

    def _gen_mask(self):
        if self.cfg.AUG.MASK_TUBE:
            num_masking_patches = round(
                np.prod(self.cfg.AUG.MASK_WINDOW_SIZE) *
            ...
            n_mask = round(self.cfg.AUG.MASK_WINDOW_SIZE[0] *
                           self.cfg.AUG.MASK_RATIO)
