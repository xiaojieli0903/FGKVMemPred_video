...
def get_start_end_idx(video_size,
                      clip_size,
                      clip_idx,
                      num_clips_uniform,
                      use_offset=False):
    ...
    if clip_idx == -1:
        ...
        if use_offset:
            if num_clips_uniform == 1:
                ...
                start_idx = clip_idx * math.floor(delta /
                                                  (num_clips_uniform - 1))
        else:
            # Uniformly sample the clip with the given index.
    ...
    def sample_clips(
        ...
        for clip_size in clip_sizes:
            for i_try in range(num_retries):
                ...
                se_inds_new = np.append(se_inds, [[start_idx, end_idx]],
                                        axis=0)
                ...
                if (any(dt < min_delta)
                        or any(dt > max_delta)) and i_try < num_retries - 1:
...
def torchvision_decode(
    ...
    if len(video_meta) > 0 and (
        ...
        for k in range(len(num_frames)):
            pts_per_frame = (video_meta["video_denominator"] /
...
def lmdb_decode(
    ...
    fps = 25  # hard code for UCF101
    frames_length = len(container)
    duration = len(container)
    ...
    clip_sizes = [
        np.maximum(
            1.0,
            np.ceil(sampling_rate[i] * num_frames[i] / target_fps * fps),
        ) for i in range(len(sampling_rate))
    ]
    start_end_delta_time = get_multiple_start_end_idx(
        frames_length,
    ...
    for k in range(len(num_frames)):
        ...
        timebase = duration / frames_length
        video_start_pts = int(start_idx * timebase)
        video_end_pts = min(len(container) - 1, int(end_idx * timebase))
        # If video stream was found, fetch video frames from the video.
        sample_index = torch.linspace(video_start_pts, video_end_pts,
                                      num_frames[0]).long()
        frames = [pil_from_raw_rgb(container[idx]) for idx in sample_index]
        frames = torch.as_tensor(np.stack(frames))
        frames_out[k] = frames
    return frames_out, fps, decode_all_video, start_end_delta_time
...
def decode(
    ...
    if num_decode > 1 and temporally_rnd_clips:
        ...
        ind_clips = np.arange(
            num_decode)  # clips come temporally ordered from decoder
    try:
        if backend == "pyav":
            # assert (
    ...
    # num_decode: clip_number
    # num_frames: clip_number * [num_frames_per_clip]
    for k in range(num_decode):
        ...
        # frames: torch.Size([48, 240, 320, 3])
        if frames.shape[0] != T:
            frames_k = temporal_sampling(frames, start_idx, end_idx, T)
        else:
            frames_k = frames
        # frames_k: torch.Size([8, 240, 320, 3])
    ...
    if num_decode > 1 and temporally_rnd_clips:
        ...
        assert all(frames_out[i].shape[0] == num_frames_orig[i]
