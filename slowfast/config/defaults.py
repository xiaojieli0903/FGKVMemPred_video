...
# The type of predictor in ['MLP', 'Memory']
_C.CONTRASTIVE.PREDICTOR_TYPE = "MLP"

_C.CONTRASTIVE.LOSS_SIM_WEIGHT = 1.0

_C.CONTRASTIVE.NORM_KEYS = True

_C.CONTRASTIVE.EXTRACT_TYPE = "keys"

# Memory configuration options
_C.MEMORY = CfgNode()

# Dimension of the source visual concept dictionary
_C.MEMORY.DIM_SOURCE_MEM = 256
# Dimension of the target visual concept dictionary
_C.MEMORY.DIM_TARGET_MEM = 256
# Dimension of the key-value memory
_C.MEMORY.DIM_CROSS_MEM = 256

# Scaling factor for softmax (radius for attention distribution scaling)
_C.MEMORY.RADIUS = 16
# Number of visual concepts in the dictionaries
_C.MEMORY.N_CONCEPT = 4096
# Number of slots for key and value memories
_C.MEMORY.N_MEMORY = 4096
# Number of attention heads of the memory
_C.MEMORY.N_HEAD = 1
# Type of loss function for feature reconstruction using dictionary learning
_C.MEMORY.RECON_LOSS_TYPE = "cosine"
# Average dimension of the reconstruction loss
_C.MEMORY.AVERAGE_DIM = 0
# Whether to use input projection before memory
_C.MEMORY.USE_INPUT_PROJ = False
# Dimension for input projection to cross memory
_C.MEMORY.DIM_CROSS_INPUT_PROJ = 256
# Whether to use output projection after memory
_C.MEMORY.USE_OUTPUT_PROJ = False
# Whether to use source feature reconstruction
_C.MEMORY.USE_SRC_RECON = False
# Whether to use target feature reconstruction
_C.MEMORY.USE_TAR_RECON = False
# Whether to use KL divergence loss between the visual concepts codes
_C.MEMORY.USE_ALIGN = True
# Temperature for KL divergence loss
_C.MEMORY.KL_T = 1.0

# Whether to use contrastive loss for source memory
_C.MEMORY.USE_SOURCE_MEM_CONTRASTIVE = False
# Whether to use contrastive loss for target memory
_C.MEMORY.USE_TARGET_MEM_CONTRASTIVE = False
# Whether to use contrastive loss for cross memory
_C.MEMORY.USE_CROSS_MEM_CONTRASTIVE = False

# Type of addressing for memory (e.g., "cosine" or other)
_C.MEMORY.ADDRESS_TYPE = "cosine"
# Whether to predict residuals for memory update
_C.MEMORY.PREDICT_RESIDUAL = False

# Whether to use sparse coding for memory
_C.MEMORY.USE_SPARSE = False
# Whether to use sparse coding before memory
_C.MEMORY.USE_SPARSE_BEFORE = False
# Regulization loss type for sparse coding
_C.MEMORY.SPARSE_LOSS_TYPE = 'topk'
# Top-k for sparse coding
_C.MEMORY.SPARSE_TOPK = 100

# Loss weights for different memory-related losses
_C.MEMORY.LOSS_SOURCE_MEM_CONTRASTIVE = 0.0001  # Weight for contrastive loss in source memory
_C.MEMORY.LOSS_TARGET_MEM_CONTRASTIVE = 0.0001  # Weight for contrastive loss in target memory
_C.MEMORY.LOSS_CROSS_MEM_CONTRASTIVE = 0.0001  # Weight for contrastive loss in cross memory
_C.MEMORY.LOSS_SOURCE_RECON = 0.0  # Weight for source feature reconstruction loss
_C.MEMORY.LOSS_TARGET_RECON = 0.0  # Weight for target feature reconstruction loss
_C.MEMORY.LOSS_SOURCE_MEM_SPARSE = 0.0  # Weight for sparse loss in source memory
_C.MEMORY.LOSS_TARGET_MEM_SPARSE = 0.0  # Weight for sparse loss in target memory
_C.MEMORY.LOSS_PREDICT = 0.0  # Weight for prediction loss
_C.MEMORY.LOSS_KL = 0.0  # Weight for KL divergence loss

# Number of mlp layers for the dictionary reconstruction branch
_C.MEMORY.NUM_MLP_LAYERS = 2
# Number of mlp layers for the source feature projection branch
_C.MEMORY.NUM_MLP_LAYERS_IN = 2
# Number of mlp layers for the output feature projection branch
_C.MEMORY.NUM_MLP_LAYERS_OUT = 2

# Type of the structure for feature reconstruction in the dictionary learning
_C.MEMORY.RECON_TYPE = "memory"
# Type of the structure of the predictor, memory for the key-value memory enhanced predictor and mlp for the mlp predictor
_C.MEMORY.CROSS_BRANCH_TYPE = "memory"
_C.MEMORY.DINO_TEMP_T = 0.05  # Temperature for target in DINO loss
_C.MEMORY.DINO_TEMP_S = 0.05  # Temperature for source in DINO loss
...
_C.TEST.VIS_INPUT = False
_C.TEST.VIS_MIDDLE = False
...
_C.RESNET.STEM_FUNC = "basic_stem"
...
_C.DATA.ROOT = ""
_C.DATA.DB_PATH = ""
# percentage of choosing one clip from one video for augmentation
_C.DATA.CHOOSE_ONE_CLIP = 0.0
