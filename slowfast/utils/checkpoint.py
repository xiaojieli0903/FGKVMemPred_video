...
def get_last_checkpoint(path_to_job, task):
    ...
    if task != "":
        names = [
            f for f in names
            if "{}_checkpoint".format(task) in f and f.endswith("pyth")
        ]
    else:
        names = [
            f for f in names
            if f.startswith("checkpoint") and f.endswith("pyth")
        ]
    if len(names) == 0:
        return None
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            if image_init:
                if ("pos_embed" in pre_train_dict.keys()
                    ...
                    if (pre_train_dict["pos_embed"].shape[1] ==
                ...
                if ("patch_embed.proj.weight" in pre_train_dict.keys()
                    ...
                    if (len(pre_train_dict["patch_embed.proj.weight"].shape)
                            == 4 and len(
                                model_dict["patch_embed.proj.weight"].shape)
                            == 5):  # img->video
                        t = model_dict["patch_embed.proj.weight"].shape[2]
                        pre_train_dict[
                            "patch_embed.proj.weight"] = pre_train_dict[
                    ...
                    elif (len(pre_train_dict["patch_embed.proj.weight"].shape)
                          == 5
                ...
                for k in pre_train_dict.keys():
                    if (any([x in k for x in qkv]) and pre_train_dict[k].shape
                            != model_dict[k].shape):
                ...
                for k in pre_train_dict.keys():
                    if ("rel_pos" in k and pre_train_dict[k].shape
                            != model_dict[k].shape):
                        # print(pre_train_dict[k].shape, model_dict[k].shape)
                        logger.info(
                        ...
                        new_pos_embed = torch.nn.functional.interpolate(
                            pre_train_dict[k].reshape(
                                1, pre_train_dict[k].shape[0],
                                -1).permute(0, 2, 1),
                            size=model_dict[k].shape[0],
                            mode="linear",
                        )
                        new_pos_embed = (new_pos_embed.reshape(
                            -1, model_dict[k].shape[0]).permute(1,
                                                                0).squeeze())
                ...
                if k in model_dict:
                    if v.size() == model_dict[k].size():
                        ...
                        if "attn.rel_pos" in k:
                            ...
                            v = v[0].t()
                            pre_train_dict_match[k] = v
                            logger.info("{} reshaped from {} to {}".format(
                                k, v_shape, v.shape))
                            ...
                            assert (model_size *
                                    model_size == model_dict[k].shape[1])
                            v = torch.nn.functional.interpolate(
                                v.reshape(1, pretrain_size, pretrain_size,
                                          -1).permute(0, 3, 1, 2),
                            ...
                            pre_train_dict_match[k] = v.reshape(
                                1, -1,
                                model_size * model_size).permute(0, 2, 1)
                            logger.info("{} reshaped from {} to {}".format(
                                k, v_shape, pre_train_dict_match[k].shape))
                        else:
