...
def compute_and_update_bn_stats(model, data_loader, num_batches=200):
    ...
    bn_layers = [
        m for m in model.modules() if any((isinstance(m, bn_type)
                                           for bn_type in (
    ...
    running_square_mean = [
        torch.zeros_like(bn.running_var) for bn in bn_layers
    ]

    for ind, (inputs, _,
              _) in enumerate(itertools.islice(data_loader, num_batches)):
        ...
        for i, bn in enumerate(bn_layers):
            ...
            running_square_mean[i] += (cur_square_mean -
                                       running_square_mean[i]) / (ind + 1)
