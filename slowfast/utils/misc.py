...
def check_nan_losses(loss):
    ...
    """
    if math.isnan(loss):
        # raise RuntimeWarning("ERROR: Got NaN losses {}".format(datetime.now()))
...
def params_count(model, ignore_bn=False):
    ...
    if not ignore_bn:
        return np.sum(
            [p.numel() for p in model.parameters() if p.requires_grad]).item()
    else:
        count = 0
...
def log_model_info(model, cfg, use_train_input=True, print_model=True):
    """
...
def plot_input_normed(
    ...
    if output_video:
        ...
        if tensor.ndim > 4 and tensor.shape[0] == 1:
            ...
            nrow = tensor.shape[1]
            tensor = tensor.reshape(shape=(-1, tensor.shape[2],
                                           tensor.shape[3], tensor.shape[4]))
        vis2 = (make_grid(tensor, padding=8, pad_value=1.0,
                          nrow=nrow).permute(1, 2, 0).cpu().numpy())
